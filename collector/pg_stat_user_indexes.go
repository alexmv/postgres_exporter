...
func (c *PGStatUserIndexesCollector) Update(ctx context.Context, instance *instance, ch chan<- prometheus.Metric) error {
	...
	for rows.Next() {
		...
		if !schemaname.Valid {
			c.log.Debug("Skipping stats on index because schemaname is not valid")
			continue
		}
		if !relname.Valid {
			c.log.Debug("Skipping stats on index because relname is not valid")
			continue
		}
		if !indexrelname.Valid {
			c.log.Debug("Skipping stats on index because indexrelname is not valid")
			continue
		}
		labels := []string{schemaname.String, relname.String, indexrelname.String}

		if !idxScan.Valid {
			c.log.Debug("Skipping stats on index because idx_scan is not valid")
			continue
		}
		if !idxTupRead.Valid {
			c.log.Debug("Skipping stats on index because idx_tup_read is not valid")
			continue
		}
		if !idxTupFetch.Valid {
			c.log.Debug("Skipping stats on index because idx_tup_fetch is not valid")
			continue
		}
